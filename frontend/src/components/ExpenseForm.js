 ...
 * - onAdded: callback invoked after a new expense is added
 */
const ExpenseForm = ({ users, onAdded }) => {
    ...
    const submit = async (e) => {
        ...
        setSuccess("");
        try {
            await postExpense({ amount: Number(amount), category, owner_id: Number(ownerId), date, is_common: isCommon });
            setAmount("");
            setCategory("");
            ...
            console.error(err);
            alert("Error adding expense: " + (err.message || err));
        } finally {
            setLoading(false);
        }
    };
        ...
        <div>
            <form onSubmit={submit}>
                <div style={{ marginBottom: 8 }}>
                    <label>
                        Amount
                        <input className="form-input" placeholder="e.g. 12.50" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} step="0.01" required />
                    </label>
                </div>
                ...
                <div style={{ marginBottom: 8 }}>
                    <label>
                        Category
                        <input className="form-input" placeholder="e.g. Groceries" type="text" value={category} onChange={(e) => setCategory(e.target.value)} required />
                    </label>
                </div>
                ...
                <div style={{ marginTop: 6 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span className="muted">{t('commonExpense')}</span>
                    </label>
                </div>
